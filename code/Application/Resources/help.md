...
Keyboard Deconvolutor discovers all the keys by probing all possible scan code numbers in Windows format and pass it through the Windows API. In several cases, different scan code numbers are converted to the same virtual keys. In this case, the correct scan code can be found by the reverse transform.
